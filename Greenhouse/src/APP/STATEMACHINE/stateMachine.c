...
uint8_t currState_User = 0;
uint8_t PrevState_User = 0;
static char Current_Value = 0;
static char Previous_Value = 0;
uint8_t UserMode = User;
...
void GreenHouse_Init()
{
    ...
    Button_Init(FanButton_Group, FanButton_Pin);       // Fan Button
    Button_Init(HeaterButton_Group, HeaterButton_Pin); // Heater Button
    Button_Init(PumpButton_Group, PumpButton_Pin);     // Pump Button
    Button_Init(UserButton_Group, UserButton_Pin);
    WaterPump_Off();
    Heater_Off();
    Fan_Off();
...
void GreenHouse_Update()
{
    if (Interrupt_Flag == 1) // to limit CPU spamming??
    {
        ...
        case GreenHouse_Automatic:
            ...
            currState_User = Button_GetState(UserButton_Group, UserButton_Pin);
            if (currState_User != PrevState_User && PrevState_User == 0 && UserMode == User)
            {
                CurrentState = GreenHouse_Configuration;
                UserMode = Admin;
            }
            else if (currState_User != PrevState_User && PrevState_User == 0 && UserMode == Admin)
            {
                CurrentState = GreenHouse_Monitoring;
                UserMode = User;
            }
            PrevState_User = currState_User;
        ...
        case GreenHouse_Manual:
        {
            ...
            if (CurrState_Fan == BUTTON_PRESSED && PrevState_Fan == 0)
            {
                Fan_Toggle();
                Heater_Off();
            }
            else if (CurrState_Heater == BUTTON_PRESSED && PrevState_Heater == 0)
            {
                Fan_Off();
                Heater_Toggle();
            }
        ...
        case GreenHouse_Configuration:
        {
            ...
            Current_Value = Button_GetState(UserButton_Group,UserButton_Pin);
            Previous_Value = 0;
